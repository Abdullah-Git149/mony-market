...
const userList = async (req, res) => {
    try{          
          if(req.params.user_id == '60b4956e6031f3174c0075f1'){
            const users = await User.find({is_blocked:0},{user_fullname:1,user_email:1});            
            return res.status(200).send(users);    
          }
          else{
        const users = await User.find({is_blocked:1, company_id: req.params.user_id},{user_fname:1,user_email:1});            
        return res.status(200).send(users);       
      }   
